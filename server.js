...
const REDIRECT_URI = "[https://apex-teams-api.onrender.com/redirect](https://apex-teams-api.onrender.com/redirect)"; // Must match exactly in Azure

// ✅ Scopes needed for Mail, Calendar & Teams meetings
const SCOPES = [
"[https://graph.microsoft.com/User.Read](https://graph.microsoft.com/User.Read)",
"[https://graph.microsoft.com/Mail.Send](https://graph.microsoft.com/Mail.Send)",
"[https://graph.microsoft.com/Calendars.ReadWrite](https://graph.microsoft.com/Calendars.ReadWrite)"
];
...
return res.status(400).send("Error: Missing authorization code in redirect. Retry /login.");
}
...
toRecipients: (req.body.toEmails || []).map(email => ({ emailAddress: { address: email } }))
}
};

try {
const graphResponse = await fetch("[https://graph.microsoft.com/v1.0/me/sendMail](https://graph.microsoft.com/v1.0/me/sendMail)", {
method: "POST",
headers: {
...
attendees: attendees,
isOnlineMeeting: req.body.isOnlineMeeting || true,
onlineMeetingProvider: req.body.onlineMeetingProvider || "teamsForBusiness"
};

try {
const response = await fetch("[https://graph.microsoft.com/v1.0/me/events](https://graph.microsoft.com/v1.0/me/events)", {
method: "POST",
headers: {
Authorization: `Bearer ${accessToken}`,
"Content-Type": "application/json"
